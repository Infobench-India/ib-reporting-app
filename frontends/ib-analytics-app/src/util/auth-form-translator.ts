...
export const getFormFieldTitle = (field: any): string =>
  field.meta.label?.text && field.meta.label?.text in translations
    ? translations[field.meta.label?.text as keyof Translations].title
    : field.meta.label?.text;

export const getFormFieldPosition = (field: UiNode): number =>
  field.meta.label?.text && field.meta.label?.text in translations
    ? translations[field.meta.label?.text as keyof Translations].position
    : Infinity;

export const getFormPlaceholder = (field: UiNode): string =>
  field.meta.label?.text && field.meta.label?.text in translations
    ? translations[field.meta.label?.text as keyof Translations].placeholder
    : "";
